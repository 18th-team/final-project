const chatApp = (function() {
    ...
    async function openPersonalChat(chat) {
        ...
        const optionsMenu = document.querySelector('.options-menu');
        if (chat.type === 'GROUP') {
            console.log('Processing group chat with participants:', chat.participants); // 그룹 채팅 디버깅 로그
            renderParticipantsList(chat);
        } else {
            console.log('Processing private chat, removing user list button and sidebar'); // 개인 채팅 디버깅 로그
            // 개인 채팅일 경우 버튼 및 사이드바 제거
            const existingButton = optionsMenu.querySelector('.user-list');
            if (existingButton) {
                existingButton.remove();
            }
